...
SELECT l.id, l.name, l.img, l.price_start, r.rate_price, COUNT(DISTINCT r.lot_id) FROM lots l JOIN rates r ON l.id = r.user_id JOIN categories c ON l.category_id = c.id GROUP BY r.lot_id ORDER BY l.date_create DESC;
